...
export function Home() {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      const res = await openai.chat.completions.create({
        model: "gpt-4o-mini",
        messages: [
          { role: "system", content: "You are a helpful assistant with deep knowledge of HTML, CSS, JavaScript, and Web Technology. You will help the user answer their questions about how websites work." },
          {
              role: "user",
              content: inputValue,
          },
      ...
      const newResponse = res.choices[0]?.message?.content || "No response from model";
  ...
  return (
    <div className="flex flex-col max-h-screen">
      <div className="flex flex-col gap-2 pb-5 max-h-[80%]"> {/* Added padding bottom here */}
          ...
          <div className="flex flex-col space-y-4">
            ...
            {responses.map((response, index) => (
              <div key={index} className="mt-4">
                <p className="text-sm max-w-lg">Q: <b>{inputs[index]}</b></p>
      ...
      <div className="fixed bottom-0 left-0 w-full px-4 py-4 bg-white border-t">
        <form onSubmit={handleSubmit}>
          <div className="flex flex-row items-center space-x-2">
            ...
            />
            <Button className="ml-2 bg-purple-500 hover:bg-purple-600 text-white font-bold py-2 px-4 rounded" type="submit">
              Send
            </Button>
          </div>
        </form>
